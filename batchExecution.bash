...
for opt_level in O0 O1 O2 O3 O4; do
    echo "nodes,density,exp,delta,compressionRatio,executionTime" > "cpgc_results_${opt_level}.csv"
    ...
    for density in 80 85 90 95 98; do
        for exp in {1..10}; do
            for delta in 0.5 0.6 0.7 0.8 0.9 1; do
                # Run cpgc algorithms
                for opt_level in O0 O1 O2 O3 O4; do
                    ./cpgc_$opt_level "$node" "$density" "$exp" "$delta" >> "cpgc_results_${opt_level}.csv"
                done
